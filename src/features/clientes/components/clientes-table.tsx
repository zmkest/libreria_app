...
export function ClientesTable({ customers, onEdit, onDelete }: Props) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          if (!val) return empty;
          return (
            <span className="block max-w-[200px] truncate" title={val}>
              {val}
            </span>
          );
        },
            ...
            return (
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={async () => {
                    setDeletingId(null);
                    await onDelete(customer.id, fullName);
                  }}
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 border-b border-brand-border">
        <div>
          <h2 className="text-xl font-bold text-brand">Clientes registrados</h2>
          <p className="text-xs text-gray-400 mt-0.5">
            {table.getFilteredRowModel().rows.length} de {customers.length} clientes
          </p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2.5 rounded-xl text-sm text-brand-dark bg-brand-input border-2 border-brand-border outline-none transition-all focus:border-brand focus:shadow-[0_0_5px_rgba(40,85,141,0.2)] w-80"
          />
        </div>
      </div>
